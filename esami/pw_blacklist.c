...
int main()
{
	...
	while (1)
	{
		...
		if (!strcmp(method, "GET")) // se ho la chiamata di fipo GET
		{
            ...
            // parte da aggiungere al proxy blacklist:
            // prendo ipClient dalla porta a cui e' legato il socket
            //   se quell'ip e' presente nella lista, posso mandare solo content type text/html
            //      tengo conto con un flag filterData
            //      
            //   leggo header del server
            //     faccio parsing header del server
            //         e prendo header Content-Type
            //      
            //     se ip client non era nella blacklist E se header non e' content type
            //         allora posso inviare la risposta normale
            //     altrimenti se ip client e' nella lista o se header e' content type
            //         allora posso inviare solo text/html
            //     altrimenti errore 401 unauthorized 
            //
            // se close connection -> chiudo la connessione
